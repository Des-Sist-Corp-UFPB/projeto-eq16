...
1. ✅ **FEITO — Fase 2/3 — aceitar/recusar pelo Discord (Interactions Endpoint).** `POST /api/discord/interactions` valida a assinatura **Ed25519** (`DISCORD_PUBLIC_KEY`, via `crypto` nativo — sem dependências) e responde `PING → PONG`; ao solicitar entrada, o bot posta um painel de **botões** (`buildCandidaturaButtons`) e, no clique, o núcleo testável `src/lib/discordInteractions.ts` reusa `src/lib/candidaturas.ts` (mesma regra do site) e registra no **audit log**. A casca HTTP só faz a verificação de assinatura. ⚠️ Para testar local, o Discord precisa alcançar o webhook: usar um **túnel** (ngrok/cloudflared) — passo a passo em [`discord-setup.md`](./discord-setup.md).
